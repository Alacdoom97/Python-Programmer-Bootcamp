...
def bright_pix(img, circles, size):
    bright_value = []
    for coords in circles[0,:]:
        # np mean function on square of 2*size on the y coordinate and on the x coordinate
...
bright_values = bright_pix(img,circles,20)
print(bright_values)
...
for i in circles[0,:]:
    ...
    count2 += 1
cv2.putText(original_image, 'Estimated Total Value: ' + str(sum(values)) + 'p', (50,100), cv2.FONT_HERSHEY_SIMPLEX,1.3,255)
